...
SnowMan::SnowMan(GLdouble radius)
{
	...
	nose->setModelMat(
		glm::translate(glm::mat4(1.0f),
			glm::vec3(0.0f, radius + upperRadius * 0.40f, upperRadius * 0.75f)) *
	...
	Cone* eye1 = new Cone(radius * 0.40, radius * 0.20, 0.0, 8, 12);
	eye1->setColor(TEAL);
	eye1->setModelMat(
		glm::translate(glm::mat4(1.0f),
			glm::vec3(-radius * 0.20f, radius + upperRadius * 0.985f, upperRadius * 0.88f)) *
		glm::rotate(glm::mat4(1.0f),
			glm::half_pi<float>(),
			glm::vec3(1.0f, 0.0f, 0.0f)));
	addEntity(eye1);

	Cone* eye2 = new Cone(radius * 0.40, radius * 0.20, 0.0, 8, 12);
	eye2->setColor(TEAL);
	eye2->setModelMat(
		glm::translate(glm::mat4(1.0f),
			glm::vec3(radius * 0.20f, radius + upperRadius * 0.95f, upperRadius * 0.88f)) *
		glm::rotate(glm::mat4(1.0f),
			glm::half_pi<float>(),
			glm::vec3(1.0f, 0.0f, 0.0f)));
	addEntity(eye2);
	...
	GLdouble hatRadius = radius * 0.55;
	GLdouble hatHeight = radius * 0.30;
	GLdouble hatCenterY = radius + upperRadius + radius * 0.6f;
	...
	addEntity(redDisk);
	GLdouble brimRadius = radius * 0.90;   // Ala más grande

	Disk* cyanDisk = new Disk(brimRadius, 0.0, 1, 12);
	...
	Disk* hatBase = new Disk(hatRadius, 0.0, 1, 12);
	hatBase->setColor(BLUE);
	hatBase->setModelMat(
		glm::translate(glm::mat4(1.0f),
			glm::vec3(0.0f, hatCenterY - hatHeight * 1.5f, 0.0f))
	);
	addEntity(hatBase);
